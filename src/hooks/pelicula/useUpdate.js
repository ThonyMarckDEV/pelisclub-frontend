...
import { show, update } from 'services/peliculaService';
...
export const useUpdate = () => {
        ...
        const loadPelicula = async () => {
            try {
                ...
                setFormData({
                    ...
                    clasificacion:     data.clasificacion      || 'TP',
                ...
                setAlert(handleApiError(err, 'No se pudo cargar la información de la película.'));
            } finally {
                setSaving(false);
            }
        };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setAlert({ type: 'success', message: 'Película actualizada correctamente.' });
            setTimeout(() => navigate('/pelicula/listar'), 1500);
        } catch (err) {
            setAlert(handleApiError(err, 'Error al actualizar la película'));
        } finally {
            setLoading(false);
        }
    };
