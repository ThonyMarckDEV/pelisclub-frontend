...
import { store } from 'services/serieService';
...
export const useStore = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await store(payload);
            setAlert({ type: 'success', message: 'Serie registrada exitosamente. Redirigiendo...' });
            setTimeout(() => navigate('/serie/listar'), 1500);
        } catch (err) {
            setAlert(handleApiError(err, 'Error al registrar la serie'));
        } finally {
            setLoading(false);
        }
    };
