...
const MovieCard = ({ pelicula, tipo = "pelicula" }) => {
    ...
    return (
        <div className="group relative w-full aspect-[2/3] rounded-sm overflow-hidden bg-[#141215]">
            {pelicula.estreno && (
                <span className="absolute top-2 left-2 z-10 px-2 py-0.5 bg-[#E8B04B] text-black text-[9px] font-black uppercase tracking-wide rounded-sm shadow-lg">
                    Estreno
                </span>
            )}
            ...
            <BotonFavorito
                variant="icon"
                peliculaId={tipo === "pelicula" ? pelicula.id : null}
                serieId={tipo === "serie" ? pelicula.id : null}
            />
