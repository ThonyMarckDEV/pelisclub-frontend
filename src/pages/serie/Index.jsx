...
import { useIndex } from 'hooks/serie/useIndex';
import Table from 'components/Shared/Tables/Table';
import PageHeader from 'components/Shared/Headers/PageHeader';
import AlertMessage from 'components/Shared/Errors/AlertMessage';
import ConfirmModal from 'components/Shared/Modals/ConfirmModal';
import { TvIcon, PencilSquareIcon, TrashIcon, StarIcon, EyeIcon, RectangleStackIcon } from '@heroicons/react/24/outline';
...
const Index = () => {
    ...
    const filterConfig = useMemo(() => [
        { name: 'search', type: 'text', label: 'Buscar (Título/Director)', placeholder: 'Ej: Stranger Things...', colSpan: 'col-span-12 md:col-span-7' },
            ...
            render: (row) => (
                <div className="flex items-center gap-3">
                    <div className="w-10 h-14 rounded-sm bg-black border border-white/10 overflow-hidden shrink-0">
                        {row.portada_url ? <img src={row.portada_url} alt={row.titulo} className="w-full h-full object-cover" /> : (
                            <div className="w-full h-full flex items-center justify-center"><TvIcon className="w-4 h-4 text-white/20" /></div>
                        )}
                    </div>
                    <div className="flex flex-col">
                        ...
                        <span className="text-xs text-white/40">{row.generos?.join(', ') || 'Sin géneros'}</span>
                    </div>
                </div>
            )
        },
        {
            ...
            render: (row) => (
                <span className="flex items-center gap-1 text-sm font-bold text-white">
                    <RectangleStackIcon className="w-4 h-4 text-white/30" /> {row.temporadas_count}
                </span>
            )
        },
        {
            header: 'Años',
            render: (row) => <span className="text-xs text-white/60">{row.anio_inicio || 'S/A'} — {row.anio_fin || 'presente'}</span>
        },
        {
            header: 'Vistas',
            render: (row) => (
        ...
        },
        {
            ...
            render: (row) => (
                <div className="flex items-center gap-2">
                    <Link to={`/serie/editar/${row.id}`} className="p-2 text-white/50 hover:text-[#E8B04B] hover:bg-white/5 rounded-sm transition-all">
                        <PencilSquareIcon className="w-5 h-5" />
                    </Link>
                    <button onClick={() => handleAskDelete(row.id)} className="p-2 text-red-400/70 hover:text-red-400 hover:bg-red-900/10 rounded-sm transition-all">
    ...
    return (
        <div className="container mx-auto p-6">
            <PageHeader title="Gestión de Series" icon={TvIcon} buttonText="+ Nueva Serie" buttonLink="/serie/agregar" />
            <AlertMessage type={alert?.type} message={alert?.message} details={alert?.details} onClose={() => setAlert(null)} />
            <Table
                columns={columns} data={series} loading={loading}
                ...
                pagination={{ ...paginationInfo, onPageChange: fetchSeries }}
            />
            {showConfirm && (
                <ConfirmModal
                    message="¿Estás seguro de eliminar esta serie? Se eliminarán también sus temporadas y episodios."
